...
def show_exam_result(request, course_id, submission_id):
    ...
    choice_ids = Submission.objects.filter(id = submission_id).values_list('choices', flat=True)
    points = 0
    for i in submission.choices.all().filter(is_correct=True).values_list('question_id'):
        points += Question.objects.filter(id=i[0]).first().grade    
    context['selected'] = choice_ids
